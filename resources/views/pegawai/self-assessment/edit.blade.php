...
<div class="space-y-4">
    <div class="flex justify-between items-center">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Edit Self Assessment</h1>
            <p class="text-sm text-gray-500">
                Periode: {{ $periodeAktif->nama ?? '-' }} | Pegawai: {{ $pegawai->nama ?? '-' }}
            </p>
        </div>
        <a href="{{ route('pegawai.self-assessment.result') }}" class="px-4 py-2 bg-gray-500 hover:bg-gray-600 text-white rounded-md text-sm">
            <i class="fas fa-arrow-left mr-2"></i> Kembali
        </a>
    </div>

    <!-- ============================================================ -->
    <!-- PERINGATAN: Jika sudah dinilai atasan, tombol edit dinonaktifkan -->
    <!-- ============================================================ -->
    @php
        $sudahDinilaiAtasan = \App\Models\PenilaianKPI::where('pegawai_id', $pegawai->id)
    ...
    @if($sudahDinilaiAtasan)
        <div class="bg-red-50 border border-red-200 rounded-lg p-4">
            <div class="flex items-center">
                ...
                <div>
                    <p class="text-red-800 font-semibold">Self Assessment Tidak Dapat Diedit!</p>
                    <p class="text-sm text-red-700">
                        Self assessment Anda sudah dinilai oleh atasan. 
                        Jika ingin melakukan perubahan, silakan hubungi atasan atau admin.
                    </p>
                </div>
            </div>
        </div>
    @else
        <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <p class="text-sm text-blue-800">
                ...
                Silakan perbarui penilaian diri Anda secara jujur dan objektif.
            </p>
        </div>
    @endif

    <!-- Form hanya ditampilkan jika belum dinilai atasan -->
    @if(!$sudahDinilaiAtasan)
    <form action="{{ route('pegawai.self-assessment.update') }}" method="POST" id="formSelfAssessment">
        @csrf
        @method('PUT')
                            ...
                            <tr>
                                ...
                                <td class="px-4 py-2">
                                    <input type="text" name="komentar_kpi[]" 
                                           value="{{ $detail['komentar_kpi'][$index] ?? '' }}"
                            ...
                            <tr>
                                ...
                                <td class="px-4 py-2">
                                    <input type="number" name="nilai_kompetensi[]" required
                                           ...
                                           value="{{ $detail['kompetensi'][$index] ?? 3 }}"
                                           class="w-24 px-2 py-1 border border-gray-300 rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500 nilai-input"
                                           data-jenis="kompetensi">
                                </td>
                                <td class="px-4 py-2">
                                    <input type="text" name="komentar_kompetensi[]" 
                                           value="{{ $detail['komentar_kompetensi'][$index] ?? '' }}"
                                           class="w-full px-2 py-1 border border-gray-300 rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500 text-sm"
                                           placeholder="Opsional">
                                </td>
                            </tr>
                            ...
                            <tr>
                                ...
                                <td class="px-4 py-2">
                                    <input type="number" name="nilai_core_values[]" required
                                           min="1" max="{{ $indikatorItem->skala_maksimal }}" step="0.5"
                                           value="{{ $detail['core_values'][$index] ?? 3 }}"
                                           class="w-24 px-2 py-1 border border-gray-300 rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500 nilai-input"
                                           data-jenis="core_values">
                                </td>
                                <td class="px-4 py-2">
                                    <input type="text" name="komentar_core_values[]" 
                                           value="{{ $detail['komentar_core_values'][$index] ?? '' }}"
                                           class="w-full px-2 py-1 border border-gray-300 rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500 text-sm"
                                           placeholder="Opsional">
                                </td>
                            </tr>
        ...
        <div class="mt-6 flex justify-end space-x-2">
            <a href="{{ route('pegawai.self-assessment.result') }}" class="px-4 py-2 bg-gray-300 hover:bg-gray-400 rounded-md">
                Batal
            </a>
            <button type="submit" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-md" id="btnSubmit">
                <i class="fas fa-save mr-2"></i> Update Self Assessment
            </button>
        </div>
    </form>
    @endif
...
@push('scripts')
<script>
    // Auto-calculate when nilai changes (hanya jika form tersedia)
    @if(!$sudahDinilaiAtasan)
