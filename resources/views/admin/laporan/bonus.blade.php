...
@section('title', 'Laporan Bonus')

@section('content')
<div class="space-y-4">
    <div class="flex justify-between items-center">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Laporan Pembagian Bonus</h1>
    ...
    @else
        ...
        <div class="bg-white rounded-lg shadow p-4">
            <form action="{{ route('admin.laporan.bonus') }}" method="GET" class="flex flex-wrap gap-4">
                ...
                <a href="{{ route('admin.laporan.bonus') }}" class="px-4 py-2 bg-gray-500 hover:bg-gray-600 text-white rounded-md">
        ...
        @if(isset($periode) && $periode)
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div class="bg-white rounded-lg shadow p-4 text-center">
                    <p class="text-2xl font-bold text-green-600">Rp {{ number_format($summary['total_bonus'] ?? 0, 0, ',', '.') }}</p>
                    <p class="text-sm text-gray-500">Total Bonus</p>
                </div>
                <div class="bg-white rounded-lg shadow p-4 text-center">
                    <p class="text-2xl font-bold text-yellow-600">Rp {{ number_format($summary['rata_rata'] ?? 0, 0, ',', '.') }}</p>
                    <p class="text-sm text-gray-500">Rata-rata Bonus</p>
                </div>
            </div>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Nilai Kinerja</th>
                            <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Predikat</th>
                            <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Bonus Akhir</th>
                        </tr>
                    </thead>
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr>
                            ...
                            <td class="px-4 py-3 whitespace-nowrap text-sm">{{ $d->nilai_kinerja ?? '-' }}</td>
                            ...
                            <td class="px-4 py-3 whitespace-nowrap text-sm font-bold text-green-600">
                                Rp {{ number_format($d->bonus_akhir ?? 0, 0, ',', '.') }}
                            </td>
                        </tr>
                        @empty
                        <tr>
        ...
        @elseif(!isset($message))
            <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                <p class="text-yellow-800">
                    <i class="fas fa-info-circle mr-2"></i>
                    Silakan pilih periode penilaian untuk melihat laporan bonus
                </p>
            </div>
        @endif
    @endif
